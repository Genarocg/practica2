//glew include
#include <GL/glew.h>
//crear movimiento para el cuerpo humano
//crear un carrito y guardar sus posiciciones
//std includes
#include <sstream>
#include <string>
#include <iostream>
...
void applicationLoop() {
	...
	//Indices del arreglo keyframes del actual y el siguiente.
	...
	while (psi) {
		...
		if (keyFramesBrazo[indexKeyFrameBrazoCurr].size() == 7 && keyFramesBrazo[indexKeyFrameBrazoNext].size() == 7) {
			...
			firstQuat = glm::quat_cast(keyFramesBrazo[indexKeyFrameBrazoCurr][0]);//indexa el frame actual y el segundo elemento del arreglo es el elemento que quieren recuperar
			secondQuat = glm::quat_cast(keyFramesBrazo[indexKeyFrameBrazoNext][0]);
			//hace la interpolaion del quaternion (matriz de rotacion)
			finalQuat = glm::slerp(firstQuat, secondQuat, interpolation);
			//se conbvierte el quaternion en una matriz de 4x4
			interpoltaedMatrix = glm::mat4_cast(finalQuat);

			//-se recupera el frame y se multiplica por unvector 0,0,0,1
			...
			//uinimos la matriz de interpolacion del quaternion y la interpolacion de traslacion
			interpoltaedMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(finalTrans)) * interpoltaedMatrix;

			// Animacion KeyFrames
			glm::mat4 matrixGlobalAnimation = glm::translate(glm::mat4(1.0f), glm::vec3(4.0f, 2.0f, 0.0f));//trasladamos donde vemos el modrlo
